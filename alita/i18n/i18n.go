...
func InitializeI18n(locales *embed.FS, localesDir string) error {
	...
	// Ensure we allow missing translations to fall back to defaults
	config.AllowMissingTranslations = true
	...
	// Pre-load English translations to ensure they're available
	if tr, err := globalTranslatorManager.GetTranslator("en"); err != nil {
		log.Warnf("Failed to pre-load English translations: %v", err)
	} else {
		log.Infof("English translator loaded with %d translations", tr.TranslationCount())
	}
...
func registerAllMessages() {
	...
	MustRegister("admin_admin_list", "Admins in <b>{chat_title}</b>:", "chat_title")
	MustRegister("admin_cache_note_fresh", "Note: These are up-to-date values")
	MustRegister("admin_cache_note_cached", "Note: These values are cached and may not be up-to-date")
	MustRegister("admin_cache_admin_required", "You need to be admin to do this!")
	MustRegister("admin_invite_link_username", "Here is the invite link of this chat: {link}", "link")
	MustRegister("admin_invite_link_private", "Here is the invite link of this chat: {link}", "link")
	MustRegister("admin_promote_success", "Successfully promoted {user}!", "user")
	MustRegister("admin_demote_success", "Successfully demoted {user}!", "user")
	...
	MustRegister("admin_title_success_set", "Successfully set {user}'s admin title to <b>{title}</b>", "user", "title")
	...
	MustRegister("admin_promote_admin_title_truncated", "Admin title truncated to 16 characters from {original_length}", "original_length")
	MustRegister("admin_anon_admin_enabled", "AnonAdmin mode is currently <b>enabled</b> for {chat_title}.\nThis allows all anonymous admin to perform admin actions without restriction.", "chat_title")
	MustRegister("admin_anon_admin_disabled", "AnonAdmin mode is currently <b>disabled</b> for {chat_title}.\nThis requires anonymous admins to press a button to confirm their permissions.", "chat_title")
	MustRegister("admin_anon_admin_enabled_now", "AnonAdmin mode is now <b>enabled</b> for {chat_title}.\nFrom now onwards, I will ask the admins to verify permissions from anonymous admins.", "chat_title")
	MustRegister("admin_anon_admin_disabled_now", "AnonAdmin mode is now <b>disabled</b> for {chat_title}.\nFrom now onwards, I won't ask the admins to verify for permissions anymore from anonymous admins.", "chat_title")
	MustRegister("admin_anon_admin_already_enabled", "AnonAdmin mode is already <b>enabled</b> for {chat_title}", "chat_title")
	MustRegister("admin_anon_admin_already_disabled", "AnonAdmin mode is already <b>disabled</b> for {chat_title}", "chat_title")
	...
	// Help module messages - Register all help messages so they can be looked up
	...
	MustRegister("antiflood_setflood_success", "Successfully set antiflood limit to {limit} messages.", "limit")
	MustRegister("antiflood_flood_disabled", "Antiflood is currently disabled in {chat_title}.", "chat_title")
	MustRegister("antiflood_flood_show_settings", "Current antiflood settings for {chat_title}:\n• Limit: {limit} messages\n• Action: {action}\n• Delete flood messages: {delete_status}", "chat_title", "limit", "action", "delete_status")
	MustRegister("antiflood_setfloodmode_success", "Successfully updated antiflood action to {action}.", "action")
	MustRegister("antiflood_setfloodmode_unknown_type", "Unknown action type: {action}", "action")
	...
	// Ban module messages
	MustRegister("bans_ban_success", "Successfully banned {user}!", "user")
	MustRegister("bans_unban_success", "Successfully unbanned {user}!", "user")
	MustRegister("bans_kick_success", "Successfully kicked {user}!", "user")
	MustRegister("bans_mute_success", "Successfully muted {user}!", "user")
	MustRegister("bans_unmute_success", "Successfully unmuted {user}!", "user")
	...
	MustRegister("connection_connected", "Connected to {chat_title}!", "chat_title")
	MustRegister("connection_disconnected", "Disconnected from {chat_title}!", "chat_title")
	MustRegister("connection_not_connected", "You are not connected to any chat.")
	MustRegister("connection_already_connected", "You are already connected to {chat_title}!", "chat_title")
	
	// Blacklist module messages
	MustRegister("blacklists_added", "Added blacklist trigger: {trigger}", "trigger")
	MustRegister("blacklists_removed", "Removed blacklist trigger: {trigger}", "trigger")
	MustRegister("blacklists_not_found", "Blacklist trigger not found: {trigger}", "trigger")
	MustRegister("blacklists_action_performed", "Blacklisted word detected! {user} has been {action}!", "user", "action")
	...
	MustRegister("error_no_permission_change_info", "You don't have permission to change info in this group!")
	MustRegister("error_no_permission_restrict", "You don't have permission to restrict users in this group!")
	MustRegister("error_bot_cannot_restrict", "I can't restrict people here! Make sure I'm admin and can restrict other members.")
	MustRegister("error_no_permission_promote", "You can't promote/demote people here! Make sure you have appropriate rights!")
	MustRegister("error_bot_cannot_promote", "I can't promote/demote people here! Make sure I'm admin and can appoint new admins.")
	MustRegister("error_no_permission_pin", "You can't pin messages here! Make sure you're admin and can pin messages.")
	MustRegister("error_bot_cannot_pin", "I can't pin messages here! Make sure I'm admin and can pin messages.")
	MustRegister("error_bot_no_invite_links", "I don't have access to invite links! Make sure I'm admin and can invite users.")
	MustRegister("error_no_permission_invite_links", "You don't have access to invite links; You need to be admin to get this!")
	MustRegister("error_no_permission_delete", "You don't have Permissions to Delete Messages!")
	MustRegister("error_bot_cannot_delete", "I don't have Permissions to Delete Messages!")
	MustRegister("error_bot_not_admin", "I'm not admin!")
	MustRegister("error_only_admins", "Only admins can execute this command!")
	MustRegister("error_only_owner", "Only group creator to do this!")
	MustRegister("error_pm_only", "This command is made for pm, not group chat!")
	MustRegister("error_group_chat_only", "This command is made to be used in group chats, not in pm!")
	
	// Extraction error messages
	MustRegister("error_chat_not_found", "failed to connect to chat: failed to get chat: unable to getChat: Bad Request: chat not found")
	MustRegister("error_need_chat_id", "I need a chat id to connect to!")
	MustRegister("error_user_not_seen", "Could not find a user by this name; are you sure I've seen them before?")
	MustRegister("error_failed_get_user", "Failed to get user: unable to getChatMember: Bad Request: chat not found")
	MustRegister("error_invalid_time_amount", "Invalid time amount specified.")
	MustRegister("error_time_max_one_year", "Cannot set time to more than 1 year!")
	MustRegister("error_invalid_time_format", "Invalid time format specified.")
	
	// Help module error for configuration
	MustRegister("error_config_private_only", "Configuration only works in private")
	...
	MustRegister("misc_current_chat_info", "Current chat ID: <code>{chat_id}</code>", "chat_id")
	...
	MustRegister("devs_update_error", "❌ Update failed: {error}", "error")
	MustRegister("devs_broadcast_success", "✅ Broadcast sent to {count} chats", "count")
	MustRegister("devs_broadcast_error", "❌ Broadcast failed: {error}", "error")
	MustRegister("devs_only_owner", "This command can only be used by the bot owner.")

	// Disabling module messages
	MustRegister("disabling_disabled_success", "Disabled <code>{command}</code> for all non-admin users in this chat.", "command")
	MustRegister("disabling_enabled_success", "Enabled <code>{command}</code> for all users in this chat.", "command")
	MustRegister("disabling_already_disabled", "Command <code>{command}</code> is already disabled.", "command")
	MustRegister("disabling_already_enabled", "Command <code>{command}</code> is already enabled.", "command")
	MustRegister("disabling_not_disableable", "Command <code>{command}</code> cannot be disabled.", "command")
	...
	MustRegister("language_current_user", "Your Current Language is {language}\nChoose a language from keyboard below.", "language")
	MustRegister("language_current_group", "This Group's Current Language is {language}\nChoose a language from keyboard below.", "language")
	MustRegister("language_changed_user", "Your language has been changed to {language}", "language")
	MustRegister("language_changed_group", "This group's language has been changed to {language}", "language")
	MustRegister("language_current", "Your current language is set to <b>{language}</b>.", "language")
	MustRegister("language_changed", "Language changed to <b>{language}</b>!", "language")
	MustRegister("language_change_failed", "Failed to change language. Please try again.")
	MustRegister("language_not_supported", "Language <code>{language}</code> is not supported.", "language")
	MustRegister("language_select_prompt", "Please select your preferred language:")
	MustRegister("language_group_changed", "Group language changed to <b>{language}</b>!", "language")
	MustRegister("language_user_changed", "Your language changed to <b>{language}</b>!", "language")
	...
	MustRegister("antispam_user_detected", "🚨 Spam detected from {user}! Taking action: {action}", "user", "action")
	MustRegister("antispam_whitelist_added", "Added {user} to antispam whitelist.", "user")
	MustRegister("antispam_whitelist_removed", "Removed {user} from antispam whitelist.", "user")
	MustRegister("antispam_action_set", "Antispam action set to <b>{action}</b>.", "action")
	...
	MustRegister("locks_invalid_type", "`{type}` is not a correct lock type, check /locktypes.", "type")
	...
	MustRegister("mutes_dmute_reply_required", "You need to reply to a message to delete it and mute the user!")
	...
	MustRegister("notes_current_notes_chat", "These are the current notes in <b>{chat_title}</b>:", "chat_title")
	...
	MustRegister("notes_click_get_private", "Click on the button below to get the note *{name}*", "name")
	...
	MustRegister("notes_saved_success", "Saved Note <b>{name}</b>!\nGet it with <code>#{name}</code> or <code>/get {name}</code>.", "name", "name", "name")
	MustRegister("notes_saved_success_conflicting_flags", "Saved Note <b>{name}</b>!\nGet it with <code>#{name}</code> or <code>/get {name}</code>.\n\n<b>Note:</b> This note will be sent to default setting of group notes, because it has both <code>{{private}}</code> and <code>{{noprivate}}</code>.", "name", "name", "name")
	MustRegister("notes_already_exists", "Note already exists!\nDo you want to overwrite it?")

	// Pins module messages
	MustRegister("pins_replied_not_pinned", "Replied message is not a pinned message.")
	MustRegister("pins_unpinned_this", "Unpinned this message.")
	MustRegister("pins_unpinned_last", "Unpinned the last pinned message.")
	MustRegister("pins_unpinned_all_success", "Unpinned all pinned messages in this chat!")
	MustRegister("pins_unpin_cancelled", "Cancelled operation to unpin messages!")
	MustRegister("pins_confirm_unpin_all", "Are you sure you want to unpin all pinned messages?")
	MustRegister("pins_reply_or_text_needed", "Please reply to a message or give some text to pin.")
	MustRegister("pins_permapin_not_supported", "Permapin not supported for data type!")
	MustRegister("pins_pinned_message", "I have pinned <a href='{link}'>this message</a>", "link")
	MustRegister("pins_reply_to_pin", "Reply to a message to pin it")
	MustRegister("pins_pinned_message_loudly", "I have pinned <a href='{link}'>this message</a> loudly!", "link")
	MustRegister("pins_antichannelpin_enabled", "<b>Enabled</b> anti channel pins. Automatic pins from a channel will now be replaced with the previous pin.")
	MustRegister("pins_antichannelpin_disabled", "<b>Disabled</b> anti channel pins. Automatic pins from a channel will not be removed.")
	MustRegister("pins_invalid_option", "Your input was not recognised as one of: yes/no/on/off")
	MustRegister("pins_antichannelpin_status_enabled", "Anti-Channel pins are currently <b>enabled</b> in {chat_title}. All channel posts that get auto-pinned by telegram will be replaced with the previous pin.", "chat_title")
	MustRegister("pins_antichannelpin_status_disabled", "Anti-Channel pins are currently <b>disabled</b> in {chat_title}.", "chat_title")
	...
	MustRegister("pins_cleanlinked_status_enabled", "Linked channel post deletion is currently <b>enabled</b> in {chat_title}. Messages sent from the linked channel will be deleted", "chat_title")
	MustRegister("pins_cleanlinked_status_disabled", "Linked channel post deletion is currently <b>disabled</b> in {chat_title}.", "chat_title")
	MustRegister("pins_no_pinned_message", "No message has been pinned in the current chat!")
	MustRegister("pins_here_pinned_message", "<a href='{link}'>Here</a> is the pinned message.", "link")
	MustRegister("pins_button_pinned_message", "Pinned Message")
	...
	MustRegister("reports_need_reply", "You need to reply to a message to report it.")
	MustRegister("reports_cant_report_own", "You can't report your own message!")
	MustRegister("reports_expose_yourself", "You need to expose yourself first!")
	MustRegister("reports_special_account", "It's a special account of telegram!")
	MustRegister("reports_admin_cant_report", "You're an admin, whom will I report your issues to?")
	MustRegister("reports_cant_report_bot", "Why would I report myself?")
	MustRegister("reports_cant_report_admin", "Why would I report an admin?")
	MustRegister("reports_user_enabled", "Turned on reporting! You'll be notified whenever anyone reports something in groups you are admin.")
	MustRegister("reports_chat_enabled", "Users will now be able to report messages.")
	MustRegister("reports_user_disabled", "Turned off reporting! You'll no longer be notified whenever anyone reports something in groups you are admin.")
	MustRegister("reports_chat_disabled", "Users will no longer be able to report via @admin or /report.")
	MustRegister("reports_group_only", "This command can only be used in a group!")
	MustRegister("reports_blocked_user", "Blocked user {mention} from reporting.", "mention")
	MustRegister("reports_reply_to_block", "You must reply to a user to block them.")
	MustRegister("reports_unblocked_user", "Unblocked user {mention} from reporting.", "mention")
	MustRegister("reports_reply_to_unblock", "You must reply to a user to unblock them.")
	...
	MustRegister("reports_user_status_enabled", "Your current preference is true, You'll be notified whenever anyone reports something in groups you are admin.")
	MustRegister("reports_user_status_disabled", "You'll have nt enabled reports, You won't be notified.")
	MustRegister("reports_chat_status_enabled", "Reports are currently enabled in this chat.\nUsers can use the /report command, or mention @admin, to tag all admins.")
	MustRegister("reports_chat_status_disabled", "Reports are currently disabled in this chat.")
	MustRegister("reports_change_setting_help", "To change this setting, try this command again, with one of the following args: yes/no/on/off")
	MustRegister("reports_action_kicked", "✅ Successfully Kicked")
	MustRegister("reports_user_kicked_by", "User kicked!Action taken by {admin}", "admin")
	MustRegister("reports_action_banned", "✅ Successfully Banned")
	MustRegister("reports_user_banned_by", "User banned!Action taken by {admin}", "admin")
	MustRegister("reports_action_deleted", "✅ Successfully Deleted")
	MustRegister("reports_message_deleted_by", "Message Deleted!Action taken by {admin}", "admin")
	MustRegister("reports_action_resolved", "✅ Resolved Report Successfully!")
	MustRegister("reports_resolved_by", "<b>Resolved by:</b> {admin}", "admin")
	...
	MustRegister("rules_private_disabled", "All /rules commands will send the rules to {chat_title}.", "chat_title")
	MustRegister("rules_invalid_option", "Your input was not recognised as one of: yes/no/on/off")
	MustRegister("rules_for_chat", "The rules for <b>{chat_title}</b> are:", "chat_title")
	MustRegister("rules_no_rules_set", "This chat doesn't seem to have had any rules set yet... I wouldn't take that as an invitation though.")
	MustRegister("rules_click_for_rules", "Click on the button to see the chat rules!")
	MustRegister("rules_need_rules_to_set", "You need to give me rules to set!\nEither reply to a message to provide them with command.")
	...
	MustRegister("pins_here_pinned", "<a href='{link}'>Here</a> is the pinned message.", "link")
	...
	MustRegister("pins_antichannel_status_enabled", "Anti-Channel pins are currently <b>enabled</b> in {chat_title}. All channel posts that get auto-pinned by telegram will be replaced with the previous pin.", "chat_title")
	MustRegister("pins_antichannel_status_disabled", "Anti-Channel pins are currently <b>disabled</b> in {chat_title}.", "chat_title")

	// Purges module messages
	MustRegister("purges_reply_to_purge", "Reply to a message to select where to start purging from.")
	MustRegister("purges_purged_messages", "Purged {count} messages.", "count")
	MustRegister("purges_purged_messages_reason", "Purged {count} messages.\n*Reason*:\n{reason}", "count", "reason")
	...
	MustRegister("rules_button_set_success", "Successfully set the rules button to: <b>{button_text}</b>", "button_text")
	...
	MustRegister("bans_restrict_admin_kicked", "{admin} kicked {user}.", "admin", "user")
	MustRegister("bans_restrict_admin_muted", "{admin} muted {user}.", "admin", "user")
	MustRegister("bans_restrict_admin_banned", "{admin} banned {user}.", "admin", "user")
	...
	MustRegister("bans_unrestrict_unmuted_by", "Unmuted by {admin}.", "admin")
	MustRegister("bans_unrestrict_unbanned_by", "Unbanned by {admin}.", "admin")
	...
	MustRegister("users_info_user", "<b>User Info:</b>\n• <b>ID:</b> <code>{id}</code>\n• <b>First Name:</b> {first_name}", "id", "first_name")
	MustRegister("users_info_user_last_name", "<b>User Info:</b>\n• <b>ID:</b> <code>{id}</code>\n• <b>First Name:</b> {first_name}\n• <b>Last Name:</b> {last_name}", "id", "first_name", "last_name")
	MustRegister("users_info_user_username", "<b>User Info:</b>\n• <b>ID:</b> <code>{id}</code>\n• <b>First Name:</b> {first_name}\n• <b>Username:</b> @{username}", "id", "first_name", "username")
	MustRegister("users_info_user_full", "<b>User Info:</b>\n• <b>ID:</b> <code>{id}</code>\n• <b>First Name:</b> {first_name}\n• <b>Last Name:</b> {last_name}\n• <b>Username:</b> @{username}", "id", "first_name", "last_name", "username")
	MustRegister("users_current_chat_id", "<b>{chat_title}</b> chat ID: <code>{chat_id}</code>", "chat_title", "chat_id")
	MustRegister("users_ping_result", "<b>Ping:</b> <code>{ping}</code>\n<b>Time:</b> {time}", "ping", "time")
	...
	MustRegister("users_translate_result", "<b>Translation ({from_lang} → {to_lang}):</b>\n{translation}", "from_lang", "to_lang", "translation")
	...
	MustRegister("users_message_count", "Total messages in this chat: <b>{count}</b>", "count")
	
	// Connection module messages
	MustRegister("connections_connected", "You are currently connected to <b>{chat_title}</b>!", "chat_title")
	...
	MustRegister("connections_connect_connected", "You are now connected to <b>{chat_title}</b>!", "chat_title")
	MustRegister("connections_connect_tap_btn_connect", "Please press the button below to connect this chat to your PM.")
	MustRegister("connections_connections_btns_admin_conn_cmds", "Available Admin commands:{commands}", "commands")
	MustRegister("connections_connections_btns_user_conn_cmds", "Available User commands:{commands}", "commands")
	...
	MustRegister("connections_reconnect_reconnected", "You are now reconnected to <b>{chat_title}</b>!!", "chat_title")
	...
	MustRegister("captcha_mode_set_success", "✅ Captcha mode set to <b>{mode}</b>", "mode")
	...
	MustRegister("captcha_timeout_set_success", "✅ Captcha timeout set to <b>{timeout} minutes</b>", "timeout")
	...
	MustRegister("captcha_action_set_success", "✅ Captcha failure action set to <b>{action}</b>", "action")
	MustRegister("captcha_welcome_message", "Welcome to {chat_title}! Please complete the captcha below to verify you're human.", "chat_title")
	MustRegister("captcha_solve_prompt", "Solve this problem: <b>{problem}</b>\n\nYou have {timeout} minutes to complete this captcha.", "problem", "timeout")
	MustRegister("captcha_text_prompt", "Enter the text shown in the image below:\n\nYou have {timeout} minutes to complete this captcha.", "timeout")
	MustRegister("captcha_correct_answer", "✅ Correct! Welcome to the group.")
	MustRegister("captcha_wrong_answer", "❌ Wrong answer. Try again. Attempts left: {attempts}", "attempts")
	MustRegister("captcha_failed_attempts", "❌ You have failed the captcha verification. You have been {action}.", "action")
	MustRegister("captcha_timeout_message", "⏰ Captcha verification timed out. You have been {action}.", "action")
	MustRegister("captcha_refresh_button", "🔄 New Problem")
	MustRegister("captcha_refreshed", "Here's a new problem for you:")
	MustRegister("captcha_refresh_limit", "You've reached the maximum number of refreshes ({limit}). Please solve the current problem.", "limit")
	MustRegister("captcha_refresh_cooldown", "Please wait {seconds} seconds before requesting a new problem.", "seconds")
	...
	MustRegister("captcha_welcome_math_image", "👋 Welcome {user}!\n\nPlease solve the problem shown in the image and select the correct answer:\n\n⏱ You have <b>{timeout} minutes</b> to answer.", "user", "timeout")
	MustRegister("captcha_welcome_text_image", "👋 Welcome {user}!\n\nPlease select the text shown in the image to verify you're human:\n\n⏱ You have <b>{timeout} minutes</b> to answer.", "user", "timeout")
	MustRegister("captcha_welcome_math_text", "👋 Welcome {user}!\n\nPlease solve this math problem to verify you're human:\n\n<b>{question} = ?</b>\n\n⏱ You have <b>{timeout} minutes</b> to answer.", "user", "question", "timeout")
	MustRegister("captcha_verification_success", "✅ {user} has been verified and can now chat!", "user")
	MustRegister("captcha_wrong_answer_action", "❌ Wrong answer! You have been {action}.", "action")
	MustRegister("captcha_wrong_answer_attempts", "❌ Wrong answer! {attempts} attempts remaining.", "attempts")

	// Warns module messages
	MustRegister("warns_warn_mode_updated", "Updated warn mode to: {mode}", "mode")
	MustRegister("warns_warn_mode_unknown", "Unknown type '{type}'. Please use one of: ban/kick/mute", "type")
	...
	MustRegister("warns_user_warned", "User {user} has {current}/{limit} warnings; be careful!", "user", "current", "limit")
	MustRegister("warns_user_warned_reason", "User {user} has {current}/{limit} warnings; be careful!\n<b>Reason</b>:\n{reason}", "user", "current", "limit", "reason")
	MustRegister("warns_limit_exceeded_kick", "That's {current}/{limit} warnings; So {user} has been kicked!", "current", "limit", "user")
	MustRegister("warns_limit_exceeded_mute", "That's {current}/{limit} warnings; So {user} has been Muted!", "current", "limit", "user")
	MustRegister("warns_limit_exceeded_ban", "That's {current}/{limit} warnings; So {user} has been banned!", "current", "limit", "user")
	MustRegister("warns_no_warnings", "This user hasn't got any warnings!")
	MustRegister("warns_user_warnings_with_reasons", "This user has {current}/{limit} warnings, for the following reasons:", "current", "limit")
	MustRegister("warns_user_warnings_no_reasons", "User has {current}/{limit} warnings, but no reasons for any of them.", "current", "limit")
	MustRegister("warns_warning_removed_by", "Warn removed by {admin}.", "admin")
	MustRegister("warns_no_warnings_to_remove", "User already has no warns!")
	MustRegister("warns_current_settings", "The group has the following settings:\n<b>Warn Limit:</b> <code>{limit}</code>\n<b>Warn Mode:</b> <code>{mode}</code>", "limit", "mode")
	...
	MustRegister("warns_limit_not_integer", "{value} is not a valid integer.", "value")
	MustRegister("warns_limit_range_error", "The warn limit has to be set between 1 and 100.")
	MustRegister("warns_limit_updated", "Warn limit settings for this chat have been updated to {limit}.", "limit")
	...
	MustRegister("filters_filter_added", "Added reply for filter word <code>{keyword}</code>", "keyword")
	...
	MustRegister("filters_filter_removed", "Ok!\nI will no longer reply to <code>{keyword}</code>", "keyword")
	MustRegister("filters_no_filters", "There are no filters in this chat!")
	MustRegister("filters_list_filters", "These are the current filters in this Chat:{filters}", "filters")
	...
	MustRegister("help_contact_pm_module", "Contact me in PM for help regarding <code>{module}</code>!", "module")
	...
	// Misc module messages
	MustRegister("misc_reply_to_someone", "Reply to someone.")
	MustRegister("misc_provide_content", "Provide some content to reply!")
	MustRegister("misc_no_user_info", "Could not find the any information about this user.")
	MustRegister("misc_owner_tag", "He is my owner!")
	MustRegister("misc_dev_tag", "He is one of my dev users!")
	MustRegister("misc_translate_need_text_and_lang", "I need some text and a language code to translate.")
	MustRegister("misc_translate_no_text", "The replied message does not contain any text to translate.")
	MustRegister("misc_translate_provide_text", "Please provide some text to translate.")
	MustRegister("misc_ping_pinging", "Pinging")
	MustRegister("misc_ping_result", "Pinged in {milliseconds} ms", "milliseconds")
	MustRegister("misc_translate_request_error", "Error making a translation request!")
	MustRegister("misc_translate_reading_error", "Reading Error: {error}", "error")
	MustRegister("misc_keyboard_removing", "Removing the stuck bot keyboard...")
	MustRegister("misc_stats_total_messages", "Total Messages in {chat_title} are: {message_count}", "chat_title", "message_count")

	// Disabling module messages
	MustRegister("disabling_unknown_command", "Unknown command to disable:\n-{command}\nCheck /disableable!", "command")
	MustRegister("disabling_no_command_specified", "You haven't specified a command to disable.")

	// Devs module messages
	MustRegister("devs_chatinfo_specify_chat", "You must specify a user to get info on")
	MustRegister("devs_chatlist_getting_list", "Getting list of chats I'm in...")
	MustRegister("devs_chatlist_database_list", "Here is the list of chats in my Database!")
	MustRegister("devs_leave_success", "Okay, I left the chat!")
	MustRegister("devs_user_already_sudo", "User is already Sudo!")
	MustRegister("devs_user_added_sudo", "Added {user} to Sudo List!", "user")
	MustRegister("devs_user_already_dev", "User is already Dev!")
	MustRegister("devs_user_added_dev", "Added {user} to Dev List!", "user")
	MustRegister("devs_user_not_sudo", "User is not Sudo!")
	MustRegister("devs_user_removed_sudo", "Removed {user} from Sudo List!", "user")
	MustRegister("devs_user_not_dev", "User is not Dev!")
	MustRegister("devs_user_removed_dev", "Removed {user} from Dev List!", "user")
	MustRegister("devs_team_no_users", "No users are added Added in Team!")
	MustRegister("devs_team_dev_users", "<b>Dev Users:</b>\n")
	MustRegister("devs_team_sudo_users", "<b>Sudo Users:</b>\n")
	MustRegister("devs_team_no_users_section", "\nNo Users")
	MustRegister("devs_stats_fetching", "<code>Fetching bot stats...</code>")
