...
func (m moduleStruct) connection(b *gotgbot.Bot, ctx *ext.Context) error {
	...
	_text := tr.Message("connections_connected", i18n.Params{"chat_title": chat.Title})
...
func (m moduleStruct) allowConnect(b *gotgbot.Bot, ctx *ext.Context) error {
	...
	if len(args) >= 2 {
		...
		switch toogleOption {
		case "on", "true", "yes":
			text = tr.Message("connections_allow_connect_turned_on", nil)
			go db.ToggleAllowConnect(chat.Id, true)
		case "off", "false", "no":
			text = tr.Message("connections_allow_connect_turned_off", nil)
			go db.ToggleAllowConnect(chat.Id, false)
		default:
			text = tr.Message("connections_allow_connect_invalid_option", nil)
		}
	} else {
		currSetting := db.GetChatConnectionSetting(chat.Id).AllowConnect
		if currSetting {
			text = tr.Message("connections_allow_connect_currently_on", nil)
		} else {
			text = tr.Message("connections_allow_connect_currently_off", nil)
		}
	}
...
func (m moduleStruct) connect(b *gotgbot.Bot, ctx *ext.Context) error {
	...
	if ctx.Message.Chat.Type == "private" {
		...
		if !db.GetChatConnectionSetting(chat.Id).AllowConnect && !chat_status.IsUserAdmin(b, chat.Id, user.Id) {
			text = tr.Message("connections_connect_connection_disabled", nil)
		} else {
			go db.ConnectId(user.Id, chat.Id)
			text = tr.Message("connections_connect_connected", i18n.Params{"chat_title": chat.Title})
		...
		if !db.GetChatConnectionSetting(chat.Id).AllowConnect && !chat_status.IsUserAdmin(b, chat.Id, user.Id) {
			...
			text = tr.Message("connections_connect_tap_btn_connect", nil)
...
func (m moduleStruct) connectionButtons(b *gotgbot.Bot, ctx *ext.Context) error {
	...
	switch userType {
	case "Admin":
		replyText = tr.Message("connections_connections_btns_admin_conn_cmds", i18n.Params{"commands": m.adminCmdConnString()})
	case "User":
		replyText = tr.Message("connections_connections_btns_user_conn_cmds", i18n.Params{"commands": m.userCmdConnString()})
	case "Main":
		chatId := m.isConnected(b, ctx, user.Id)
		if chatId == 0 {
			return ext.EndGroups
		...
		replyText = tr.Message("connections_connected", i18n.Params{"chat_title": pchat.Title})
...
func (m moduleStruct) disconnect(b *gotgbot.Bot, ctx *ext.Context) error {
	...
	if ctx.Message.Chat.Type == "private" {
		...
		text = tr.Message("connections_disconnect_disconnected", nil)
	} else {
		text = tr.Message("connections_disconnect_need_pm", nil)
	}
...
func (m moduleStruct) isConnected(b *gotgbot.Bot, ctx *ext.Context, userId int64) int64 {
	...
	text := tr.Message("connections_not_connected", nil)
...
func (m moduleStruct) reconnect(b *gotgbot.Bot, ctx *ext.Context) error {
	...
	if ctx.Message.Chat.Type == "private" {
		...
		if chatId != 0 {
			...
			text = tr.Message("connections_reconnect_reconnected", i18n.Params{"chat_title": gchat.Title})
			connKeyboard = helpers.InitButtons(b, gchat.Id, user.Id)
		} else {
			text = tr.Message("connections_reconnect_no_last_chat", nil)
		}
		...
		text := tr.Message("connections_reconnect_need_pm", nil)
		_, err := msg.Reply(b, text, helpers.Shtml())
		if err != nil {
			log.Error(err)
			return err
